...
hydro_COPA <- results %>% filter(name == "x_hydro" & reg == "SE002" )
...
term_COPA <- results %>% filter(name == "x_term" & reg == "SE002")
...
wind_COPA <- results %>% filter(name == "x_renew" & reg == "SE002" & iTechnology == "Wind")
...
comparing_generation_s <- ggplot() + 
  geom_line(data = gen_s_12, aes(x = Date, y = production, col = iTechnology ), size = 1) +
  scale_fill_brewer(palette="Set2") + facet_wrap(~iTechnology, ncol = 2) +
  ylab("GWh") + xlab("") +  ggtitle("Comparing generation - S" ) + 
  theme(plot.title = element_text(hjust = 0.5))
plot(comparing_generation_s)
#ggsave("C:/Users/cancella/Google Drive/!IIASA/COPA/runs/Validation_2012/figures/S",comparing_generation_ne,width=30,height=20,units="cm")
...
# area graph
comparing_generation_s_area <- ggplot(data = gen_s_12_area, aes(x = Date, y = production, fill = type)) + 
  geom_area() +
  scale_fill_brewer(palette="Set2") + facet_wrap(~iTechnology) + 
  ylab("GWh") + xlab("") +  ggtitle("Comparing generation_area - S" ) + 
  theme(plot.title = element_text(hjust = 0.5))
plot(comparing_generation_s_area)
...
points_hydro_date_s <- ggplot(data = points_hydro_s_12, aes(x=Date,y=production)) + geom_point(aes(col=iTechnology)) +
  ylab("ONS x COPA") + xlab("") +  ggtitle("Hydro - S" ) + 
  theme(plot.title = element_text(hjust = 0.5))
plot(points_hydro_date_s)
...
# thermal
points_thermal_ons  <- gen_s_12 %>% filter(iTechnology == "Thermal_ons")
points_thermal_copa <- gen_s_12 %>% filter(iTechnology == "Thermal_copa")
points_thermal_s_12 <- bind_rows(points_thermal_ons,points_thermal_copa)
points_thermal_date_s <- ggplot(data = points_thermal_s_12, aes(x=Date,y=production)) + geom_point(aes(col=iTechnology)) +
  ylab("ONS x COPA") + xlab("") +  ggtitle("Thermal - S" ) + 
  theme(plot.title = element_text(hjust = 0.5))
plot(points_thermal_date_s)
...
# wind
points_wind_ons  <- gen_s_12 %>% filter(iTechnology == "wind_ons")
points_wind_copa <- gen_s_12 %>% filter(iTechnology == "wind_copa")
points_wind_s_12 <- bind_rows(points_wind_ons,points_wind_copa)
points_wind_date_s <- ggplot(data = points_wind_s_12, aes(x=Date,y=production)) + geom_point(aes(col=iTechnology)) +
  ylab("ONS x COPA") + xlab("") +  ggtitle("wind - S" ) + 
  theme(plot.title = element_text(hjust = 0.5))
plot(points_wind_date_s)
#ggsave("C:/Users/cancella/Google Drive/!IIASA/COPA/runs/Validation_2012/figures",points_thermal_date_se,width=30,height=20,units="cm")
...
stats_summary_s
#setwd("C:/Users/cancella/Google Drive/!IIASA/COPA  Initial Data/ONS validation")
#write_feather(stats_summary_s,"stats_summary_s.feather")
