...
# Checking the values of 2012 of wind_br.feather
wind_2012 <- read_feather("wind_br.feather")
dates_2012 <- wind_2012 %>% filter(year(wind_2012$Date) == 2012)
sum(is.na(dates_2012$capFact))
FC_mean <- dates_2012 %>% group_by(R) %>% summarise(FC_mean = mean(capFact))
FC_max <- dates_2012 %>% group_by(R) %>% summarise(FC_max = max(capFact))
FC_min <- dates_2012 %>% group_by(R) %>% summarise(FC_min = min(capFact))
FC_stat <- bind_cols(FC_mean, as_tibble(FC_max$FC_max),as_tibble(FC_min$FC_min))  
colnames(FC_stat) <- c("R", "FC_mean", "FC_max", "FC_min")

ggplot(FC_stat)+
  geom_bar(aes(x = R , y = FC_mean), stat ="identity") +
  geom_point(aes(x = R, y = FC_max)) +
  geom_point(aes(x = R, y = FC_min))