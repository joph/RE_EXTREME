# This script creates the hydro files for Brazilian version of COPA. 
...
hydro_data_br <- se_hydro
hydro_data_br$Region <- c("BR1", "BR2", "BR3", "BR4", "BR1")

# maxReservoir
# http://www.ons.org.br/tabela_reservatorios/conteudo.asp Acessed: July, 24th, 2017.
# The maximum reservoir capacity is given in MWmes and I'll pass to GW by 
#http://www.ons.org.br/download/biblioteca_virtual/publicacoes/dados_relevantes_2009/09-Siglas-e-Unidades-de-Medidas-Utilizadas.html
# MWmed: Megawatt médio - 1 MWmed-ano = 8.760 MWh/ano (energia média no intervalo de tempo considerado)
# MWmês: Megawatt mês - 1 MW/mês = 720 MWh/mês (medida de armazenamento) 
# MWh = MWmes x 720
# data in MWmes from July, 27th
MWmonth <- c(205002, 19873, 51859, 14812)
maxreservoir_MWh <- MWmonth * 720  # It is not necessary to multiply by 12. 
hydro_data_br$maxReservoir <- c(maxreservoir_MWh,0) 
...
hydro_data_br$maxHydPower <- c(maxHydPower$maxHydPower[3],
...
hydro_data_br$minFlow <- c(minFlow$minFlow[c(4,3,2,1)],0) 

#### changing region names for the time being
hydro_data_br$Region[hydro_data_br$Region == "BR1"] <- "SE1"
hydro_data_br$Region[hydro_data_br$Region == "BR2"] <- "SE2"
hydro_data_br$Region[hydro_data_br$Region == "BR3"] <- "SE3"
hydro_data_br$Region[hydro_data_br$Region == "BR4"] <- "SE4"
#setwd("C:/Users/cancella/Google Drive/!IIASA/COPA/data/hydro")
#setwd("C:/Users/Rafael/Desktop/Google Drive @PPE/!IIASA/COPA/data/hydro")
#write.table(hydro_data_br, file = "hydro_data_br_1.csv", sep =";", row.names = FALSE )
