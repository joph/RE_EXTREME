...
residual_scenarios <-  bind_rows(load_adapted,reanalysis_wind,solar_adapted)  %>% 
  filter(between(year(date),2007,2014)) %>% 
  group_by(date,variable,region) %>% 
  summarize(sum(mwh)) %>% 
  mutate(mwh=`sum(mwh)`) %>% 
  ...
  group_by(date) %>% 
  summarize(Mix=sum(load)-addSolar*0.5*sum(solar)-addWind*0.5*sum(wind_merra),
            Wind=sum(load)-addWind*sum(wind_merra),
            Solar=sum(load)-addSolar*sum(solar)) %>% 
...
figure05.1<- residual_scenarios %>% filter(scenario=="Wind") %>% 
